...
const FrontPage = () => {
    ...
    return (
        <div className='p-4 mb-8'>
            <h1 className='text-3xl pt-4 font-bold flex justify-center text-kartAI-blue mb-8'>Informasjon om byggesøknaden</h1>
...
<h2 className='text-lg font-medium mb-2'>Har du alt på plass?</h2>
                        <ul className='space-y-2'>
                            {checklist.map((item, index) => (
                                <li
                                    key={index}
                                    className={`flex items-center p-2 rounded-lg cursor-pointer transition-colors ${checkedItems[index] ? 'bg-blue-200' : 'hover:bg-blue-50'}`}
                                    onClick={() => toggleCheck(index)}
                                >
                                    <div className={`w-5 h-5 border-2 rounded mr-3 flex items-center justify-center ${
                                                checkedItems[index] 
                                                    ? 'bg-kartAI-blue border-kartAI-blue text-white' 
                                                    : 'border-gray-400'
                                            }`}>
                                        {checkedItems[index] && (
                                    ...
                                    <span className={checkedItems[index] ? 'font-medium' : ''}>{item}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
