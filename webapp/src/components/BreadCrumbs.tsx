...
const NextBreadcrumb = ({
  ...
  const resetHistory = useCallback((path: string) => {
    const newHistory = [
      { url: "/", alias: "Home" },
      { url: path, alias: computeAlias(path) },
    ];
  ...
  }, [pathname, resetHistory, updateHistory]);
  ...
  return (
    <div className="sticky top-[60px] z-40">
      {history.length > 1 && (
        <ul className="text-md relative hidden text-secondary-black backdrop-blur-sm md:flex md:px-2 md:py-2">
          <li className="mx-2 ml-10 font-light hover:underline">
