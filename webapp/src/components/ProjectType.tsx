...
const ProjectType: React.FC<PageProps> = ({
  ...
  // Extract map-related state and handlers
  ...
  // Define initial form data based on external data or defaults
  const initialFormData = React.useMemo((): ProjectTypeFormData => ({
  ...
  const validateForm = React.useCallback((data: ProjectTypeFormData): boolean => {
    const isCheckboxValid = (data.projectType === "Bygge" || data.projectType === "Rive") 
  ...
  const { 
    ...
    setFormData, 
    isValid,
  } = FormService.useForm<ProjectTypeFormData>(
    initialFormData,
    validateForm
  );
  ...
  // Refactored checkbox change handler
  const handleCheckboxChange = (checkboxValue: string) => {
    ...
    const updatedSubType = getSubTypeFromSelection(formData.projectType, newCheckboxes);
    if (updatedSubType) {
  ...
  const handleNext = async () => {
    ...
    try {
      // Save the description to application fields
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full mt-10 w-full max-w-[900px] mx-auto">
      ...
      <div className="border-2 border-gray-400 rounded-lg mt-4 p-4 w-full">
        ...
        {(formData.projectType === "Bygge" || formData.projectType === "Rive") && (
          <div className="mb-4">
            <p className="text-sm mb-2">Hva vil du {formData.projectType === "Bygge" ? "bygge" : "rive"}?</p>
