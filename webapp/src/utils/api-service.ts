...
// Import the actual input type generated by tRPC from your router definition
// Replace 'ApplicationUpdateInput' with the correct generated type name if different
import { type RouterInputs } from '~/trpc/react'; // Adjust path if needed
...
export const ApplicationService = {
  ...
  useUpdateApplication: () => {
    ...
    return {
      ...mutation,
      // Use the imported input type, excluding the applicationID which is passed separately
      updateApplication: async (applicationID: number, data: Omit<ApplicationUpdateInput, 'applicationID'>) => {
        try {
          // Pass the data matching the expected input type
          await mutation.mutateAsync({
            applicationID,
            ...data,
            // updatedDate is likely optional in the input, set it here if needed
            // or ensure it's part of the 'data' object if required by the input type
...
useSaveFormData: (
  ...
  debounceMs = 1000 // Remove redundant type annotation : number
) => {
  ...
  const saveField = React.useCallback(async (fieldName: string, fieldValue: string) => {
    ...
    // Wrap async logic to ensure the function passed to setTimeout returns void
    timeoutRef.current = setTimeout(() => { 
      ...
      void executeSave(); // Execute the async function but don't return the promise
    }, debounceMs);
